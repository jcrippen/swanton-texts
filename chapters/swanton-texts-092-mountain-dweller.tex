...
\citeauthor{swanton:1909} transcribes the word for ‘dish’ as \orth{s!ī′q!} in (\lastx) which implies a form like \fm[?]{sʼéexʼ}.
This word is otherwise only documented as \fm{sʼíxʼ}.
It is not clear if this is an error – perhaps he meant to write \orth{s!î′q!} – or if the speaker really did use a long vowel here.
Because this is a very common and well documented word, the form in (\lastx) is given as \fm{sʼíxʼ} with the implied assumption that \citeauthor{swanton:1909}’s transcription is incorrect.
...
\begingl
	...
	\glc	just {} \xx{cmpv}- \xx{irr}- \rt[¹]{long} -\xx{var} \·\xx{nmz} {}
		\xx{foc} -\xx{dist}
...
It is modified by the PP and adverbless motion derivation (\fm{n}; \fm{yoo=i-…-k} repetitive) ‘laterally, horizontally’ which provides the \fm{n}-conjugation class so that the habitual selects the \fm{n-} conjugation prefix.
